...
class ShutterController:
    ...
    def __init__(
        ...
        self._listeners: list[Callable[[ShutterProfile, str, int], None]] = []
    ...
    async def _async_move_cover(
        ...
        if profile.window_sensors and position < profile.open_position:
            ...
            for sensor in profile.window_sensors:
                ...
                if sensor_state and sensor_state.state in open_states:
                    _LOGGER.warning(
                        "Window sensor %s reports open; skipping close for %s",
                        sensor,
                        profile.entity_id,
                    )
                    return
        ...
        self._notify_listeners(profile, reason, position)

    def _notify_listeners(
        self, profile: ShutterProfile, reason: str, position: int
    ) -> None:
        """Update registered listeners such as sensors."""

        for listener in self._listeners:
            listener(profile, reason, position)

    def register_listener(
        self, callback: Callable[[ShutterProfile, str, int], None]
    ) -> Callable[[], None]:
